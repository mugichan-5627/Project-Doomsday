...
def main():
    ...
    if launch:
        with st.status("☣️ Activating Doomsday Swarm...", expanded=True) as status:
            ...
            risks = run_intelligence_gathering(ai_client, tavily_client, model, ticker, company, world_state)
            st.write(f"✅ Identified {len(risks)} potential risks")
            
            # Step 5: Debate
            st.write("🗣️ Fracture Tribunal — Adversarial debate in progress...")
            verdicts = run_adversarial_debate(ai_client, model, ticker, company, risks, world_state)
